...
DataUARTHandler::DataUARTHandler(ros::NodeHandle* nh) : currentBufp(&pingPongBuffers[0]) , nextBufp(&pingPongBuffers[1]) {
    ...
    DataUARTHandler_pub = this->create_publisher<sensor_msgs:msg::PointCloud2>("/ti_mmwave/radar_scan_pcl", 100);
    radar_scan_pub = this->create_publisher<ti_mmwave_rospkg::RadarScan>("/ti_mmwave/radar_scan", 100); //problemas para agregar un mensaje personalizado
    marker_pub = this->create_publisher<visualization_msgs::msg::Marker>("/ti_mmwave/radar_scan_markers", 100); 
...
void *DataUARTHandler::readIncomingData(void)
{
    ...
    try
    {
        ...
        try
        {
            ...
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"DataUARTHandler Read Thread: Failed second time to open Data serial port, error: %s", e1.what());
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"DataUARTHandler Read Thread: Port could not be opened. Port is \"%s\" and baud rate is %d", dataSerialPort, dataBaudRate);
    ...
    if(mySerialObject.isOpen())
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"),"DataUARTHandler Read Thread: Port could not be opened");
        ...
        case READ_OBJ_STRUCT:
            ...
            while( i < mmwData.numObjOut ) {
                if (((mmwData.header.version >> 24) & 0xFF) < 3) { // SDK version is older than 3.x
                    ...
                    radarscan.header.stamp = rclcpp::Time::now();
        ...
        case CHECK_TLV_TYPE:
            ...
            if(tlvCount++ >= mmwData.header.numTLVs)  // Done parsing all received TLV sections
            {
                ...
                if (mmwData.numObjOut > 0)
                {
                    ...
                    //RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"mmwData.numObjOut after = %d", mmwData.numObjOut);
...
void DataUARTHandler::start(void)
{
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"DataUARTHandler Read Thread joined");
    pthread_join(iret2, NULL);
    //ROS_INFO("DataUARTHandler Sort Thread joined");
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"DataUARTHandler Sort Thread joined");
    pthread_join(iret3, NULL);
    //ROS_INFO("DataUARTHandler Swap Thread joined");
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"),"DataUARTHandler Swap Thread joined");
